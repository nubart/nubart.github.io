/* ============================================================
   ...
   Used by: es/audioguias/cobrar-audioguia-codigo-qr.html
...
(function () {
    ...
    var int = new Intl.NumberFormat('es-ES', { maximumFractionDigits: 0 });
    ...
    function calc() {
        ...
        var be = o('r-be');
        if (margin <= 0) {
            be.innerHTML = 'Fije un precio de venta superior al coste de 1 € por tarjeta para obtener margen.';
            return;
        }
        ...
        if (cards > 0) {
            ...
            t2 = m <= 12
                ? (' — aproximadamente <b>' + int.format(Math.ceil(m)) + ' meses</b> con su número de visitantes.')
                : (' — aproximadamente <b>' + (Math.round(m / 12 * 10) / 10) + ' años</b> con su número de visitantes.');
        }
        be.innerHTML = 'Amortización tras <b>' + int.format(beCards) + ' tarjetas vendidas</b>' + t2;
    }
