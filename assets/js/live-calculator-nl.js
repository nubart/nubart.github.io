// ---- Nubart LIVE Cost Calculator — NL ----
const LIVE_RATE = 39;
...
function liveCalcStep1Next() {
    ...
    if (pkg === 'pyo') {
        ...
        if (!qty || qty < 3000) {
            hint.textContent = 'De minimumbestelling voor Custom-kaarten is 3.000.';
            hint.style.color = '#dc3545';
            el.classList.add('is-invalid'); return;
        }
        if (qty % 1000 !== 0) {
            hint.textContent = 'Voer een veelvoud van 1.000 in (bijv. 3000, 4000, 5000\u2026).';
            hint.style.color = '#dc3545';
            el.classList.add('is-invalid'); return;
        }
        if (qty > 10000) {
            hint.innerHTML = 'Voor bestellingen van meer dan 10.000 kaarten kun je <a href="../../contact.html">contact opnemen met ons verkoopteam</a> voor een offerte op maat.';
...
function liveCalcCompute() {
    ...
    if (s.pkg === 'pyo') {
        ...
        cardLabel = 'Print-Your-Own (installatiekosten + ' + s.pkgQty + ' codes)';
    } else if (s.pkg === 'basic') {
        cardCost = s.pkgQty * LIVE_BASIC_PRICE;
        cardLabel = s.pkgQty === 1 ? 'Basic (500 kaarten)' : 'Basic (' + s.pkgQty + ' \u00d7 500 kaarten)';
    } else if (s.pkg === 'custom') {
        cardCost = liveCustomPrice(s.pkgQty) || 0;
        cardLabel = 'Custom (' + s.pkgQty.toLocaleString('nl') + ' kaarten)';
    }
    ...
    if (s.media && s.mediaQty > 0) {
        const ml = s.mediaQty === 1 ? 'Mediabibliotheek (1)' : 'Mediabibliotheken (' + s.mediaQty + ')';
    ...
    if (s.translate) {
        rows += '<li class="d-flex justify-content-between border-bottom pb-1 mb-1"><span class="text-muted">Vertaling (' + totalStreamHours + ' stream-h \u00d7 \u20ac' + LIVE_RATE + ')</span><strong>\u20ac' + (totalStreamHours * LIVE_RATE).toLocaleString('de-DE') + '</strong></li>';
        if (s.firstTime) {
            rows += '<li class="d-flex justify-content-between border-bottom pb-1 mb-1" style="color:#059669;"><span>Aftrek gratis proef (30 min)</span><strong>\u2212 \u20ac' + (trialDeduction * LIVE_RATE).toLocaleString('de-DE') + '</strong></li>';
        }
    }
    ...
    if (s.media && s.mediaQty > 0) ch += '<span style="' + cs + '">' + (s.mediaQty > 1 ? s.mediaQty + ' mediabibliotheken' : '1 mediabibliotheek') + '</span>';
    if (s.translate) ch += '<span style="' + cs + '">' + s.guides + ' gids' + (s.guides > 1 ? 'en' : '') + ' \u00b7 ' + s.hours + ' h vertaling</span>';
    else ch += '<span style="' + cs + '">Geen vertaling</span>';
    if (s.firstTime && s.translate) ch += '<span style="background:#dcfce7;color:#059669;border:1px solid #059669;border-radius:2rem;padding:0.25rem 0.75rem;font-size:0.78rem;font-weight:600;">Proef 30 min afgetrokken</span>';
    document.getElementById('live-calc-summary-chips').innerHTML = ch;

    // Vertaalnotitie tonen/verbergen
...
function liveCalcRequestQuote() {
    ...
    copyBtn.onclick = function() {
        const fullText = 'Aan: info@nubart.eu\nOnderwerp: ' + subject + '\n\n' + body;
        navigator.clipboard.writeText(fullText).then(function() {
            var btn = document.getElementById('live-calc-quote-copy');
            var original = btn.textContent;
            var copiedText = btn.getAttribute('data-copied-text') || '\u2713 Gekopieerd!';
